...
function animation1()
{
    /* 
        ...
        les keyframes peuvent prendre deux formes, la première est un tableau d'objet :
            ...
            「animate」メソッドは 2 つの引数を取ります。
            1 つ目は、HTML 要素が反復処理する必要があるキーフレームのリストです。
            2 番目はアニメーション オプションを含むオブジェクトです。

            キーフレームには 2 つの形式があり、1 つ目はオブジェクトの配列です。
    */
...
function animation3()
{
    ...
    //新しい機能にアクセスするには、「animate」オブジェクトを取得します。:
...
// 「animate」メソッドは、次のオブジェクトを使用するためのショートカットです。
