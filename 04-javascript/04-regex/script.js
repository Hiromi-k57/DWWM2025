...
/* 
    ...
    最初に表示されるのは「myRegex.test(myString);」です。正規表現で検索したものが見つかったかどうかに応じてブール値を返します。

    最初の正規表現は文字列に「または」が存在するかどうかをチェックします
*/
...
const r3 = /^ou/;
// ^ indique que cela doit commencer par "ou" /^ は「ou」で始まる必要があることを示します
console.log(r3, r3.test("Bonjour"), r3.test("outre"));

const r4 = /ou$/;
// $ indique que cela doit terminer par "ou" 「ou」で終わる必要があることを示します。
console.log(r4, r4.test("Bonjour"), r4.test("mou"));

const r5 = /ou|oi/;
// | est un "or", ici on veut soit "ou" soit "oi" |は「or」ですが、ここでは「ou」か「oi」のどちらかが必要です。
console.log(r5, r5.test("Bonjour"), r5.test("Bonsoir"));

const r6 = /[a-z]/;
// const r6 = /[a-zA-Z]/;
// vérifie la présence de caractère entre a et z (donc n'importe quelle lettre de l'alphabet) aからzまでの文字（つまりアルファベットの任意の文字）の存在を確認します。
console.log(r6, r6.test("Bonjour"), r6.test("0612345678"));

const r7 = /[^a-z]/;
// "^" entre "[]" indique que les caractères ne doivent pas être présent "[]" の間にある "^" は、文字が存在してはならないことを示します
console.log(r7, r7.test("bonjour"), r7.test("0612345678"));

const r8 = /(ou)?/;
// le "?" indique que l'élément précédent doit être présent 0 ou 1 fois 「？」前の要素が0回または1回存在する必要があることを示します
console.log(r8, r8.test("Bonjour"), r8.test("Pizza"));

const r9 = /(ou)+/;
// Le "+" indique que l'élément précédent doit être présent 1 fois ou plus 「+」は、前の要素が1回以上存在する必要があることを示します。
console.log(r9, r9.test("Bonjour"), r9.test("Pizza"));

const r10 = /(ou)*/;
// Le "*" indique que l'élément précédent doit être présent 0 fois ou plus「*」は、前の要素が0回以上存在する必要があることを示します。
console.log(r10, r10.test("Bonjour"), r10.test("Pizza"));

const r11 = /^(cou){2}$/;
// "{}" avec un chiffre, indique le nombre de fois où l'élément précédent doit apparaître. 数字を含む「{}」は、前の要素が出現する回数を示します。
console.log(r11, r11.test("coucou"), r11.test("coup"));

const r12 = /(cou){1,3}/;
// Ici j'indique un minimum et un maximum de fois où l'élément doit apparaître. 要素が出現する最小回数と最​​大回数を指定します。
console.log(r12, r12.test("coucou"), r12.test("coup"));

const r13 = /(cou){2,}/;
// sans chiffre après la virgule, on indique un minimum mais pas de maximum ,以下の数字がない場合、最小値は示されますが、最大値は示されません。
...
// "\" suivi d'un chiffre, indique que l'on souhaite voir le même contenu qu'une parenthèse précédente.「\」の後に数字を続けると、前の括弧と同じ内容を表示することを示します。
...
// Par défaut, les regex sont sensible à la casse (minuscule ou majuscule) デフォルトでは、正規表現は大文字と小文字を区別します（小文字または大文字）
...
// Si on veut les acceptés, il faut les écrires :これらを受け入れたい場合は、次のように記述する必要があります。
